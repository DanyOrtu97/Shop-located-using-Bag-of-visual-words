function overall_accuracy =  compute_accuracy(data,labels_test,labels_pred,classes,method_name,desc_test,visualize_confmat,visualize_res)
    for i=1:length(data)
            ind=find(labels_test==i);
            acc_class=mean(labels_pred(ind)==labels_test(ind));
    end
    overall_accuracy=mean(labels_pred==labels_test);
    ...
    overall_accuracy = mean(diag(CMnorm));
    fprintf('OVERALL %s classification accuracy: %1.4f\n\n',method_name,overall_accuracy);
